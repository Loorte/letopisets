...
- Перевод: `nameOriginal` / `deityOriginal` хранят оригиналы из FMG, `name` / `deity` — русские версии

## Сервисы
- `packages/server/src/services/anthropic.ts` — ленивый Anthropic-клиент (из ANTHROPIC_API_KEY)
- `packages/server/src/services/translator.ts` — батчевый перевод через Claude Haiku
- `packages/server/src/services/fmg-parser.ts` — парсер FMG JSON
